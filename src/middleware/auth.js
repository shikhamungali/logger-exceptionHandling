...
const authorisation = async function (req, res, next) {
    try {
        ...
        if (!mongoose.isValidObjectId(blogToBeModified)) {
            return res.status(403).send({ status: false, msg: "invalid blogId format" });
        }
        //================================= to check authority ===========================================
        let blog = await blogModel.findById({ _id: blogToBeModified })
        if (blog) {
            if (blog.authorId != req.loggedInAuthorId) {
                return res.status(401).send({ status: false, msg: 'Author loggedIn is not allowed to modify the requested data' })
            } else {
                next()
            }
        }
...
const authorisationQuery = async function (req, res, next) {
    try {
        ...
        if (!(Object.keys(dataQuery).length === 0)) {
            ...
            if (dataToDelete.length > 0) {
                ...
                if (!blogAuth.includes(req.loggedInAuthorId)) {
                    return res.status(401).send({ status: false, msg: 'Author loggedIn is not allowed to modify the requested data of other author' })
                }
                next()
            }
            else{
        ...
        //================== no entry in query params ==========================================
            ...
            return res.status(400).send({ status: false, message: "please provide filters to fetch data to be deleted" })
        }
    }
    catch (error) {
        return res.status(500).send({ status: false, Error: error.message })
